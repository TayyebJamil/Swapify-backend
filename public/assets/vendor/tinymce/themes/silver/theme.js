...
;(function () {
  ...
  const make$6 = (detail, _rawUiSpec) => {
    ...
    const keyOnItem = (f) => (container, simulatedEvent) =>
      closest$1(simulatedEvent.getSource(), '.' + detail.markers.item).bind(
        (target) =>
          container
            .getSystem()
            .getByDom(target)
      ...
      [
        run$1(focus(), (sandbox, simulatedEvent) => {
          ...
          layeredState.lookupItem(getItemValue(item)).each(() => {
            ...
            layeredState
              .refresh(value)
              .each((path) => closeOthers(sandbox, layeredState, path))
          })
        }),
        runOnExecute$1((component, simulatedEvent) => {
    ...
    switch (item.type) {
      ...
      case 'fancymenuitem':
        return createFancyMenuItem(item).fold(handleError, (d) =>
          fancy(parseForHorizontalMenu(d), backstage)
        )
      default: {
  ...
  const renderImagePreview = (spec, initialData) => {
    ...
    const setValue = (frameComponent, data) => {
      ...
      memImage.getOpt(frameComponent).each((imageComponent) => {
        ...
        if (
          !isUndefined(translatedData.cachedWidth) &&
          !isUndefined(translatedData.cachedHeight)
        ) {
  ...
  const renderReplacableIconFromPack = (iconName, iconsProvider) =>
  ...
  const unwrapReferences = (items, menuItems) => {
      ...
      (acc, item) => {
        if (isMenuItemReference(item)) {
          if (item === '') {
            ...
            return acc.length > 0 && !isSeparator$1(acc[acc.length - 1])
    ...
    if (
      realItems.length > 0 &&
      isSeparator$1(realItems[realItems.length - 1])
    ) {
      realItems.pop()
    }
    return realItems
  ...
  const getFromItem = (item, menuItems) =>
    isExpandingMenuItem(item)
          ...
          if (focusInInput) {
            if (get$6(component.element).length >= detail.minChars) {
              const previousValue = Composing.getCurrent(sandbox).bind((menu) =>
                Highlighting.getHighlighted(menu).map(Representing.getValue)
              )
    ...
    renderCommonDropdown(
      {
        ...
        fetch: (_comp, callback) => {
          spec.fetch((items) => {
            callback(
              build(items, ItemResponse$1.CLOSE_ON_EXECUTE, backstage, false)
            )
          })
        },
        onSetup: spec.onSetup,
  ...
  const renderFromBridge =
    (bridgeBuilder, render) => (spec, extras, editor) => {
  ...
  const types = {
    button: renderFromBridge(createToolbarButton, (s, extras) =>
      renderToolbarButton(s, extras.backstage.shared.providers)
    ),
    togglebutton: renderFromBridge(createToggleButton, (s, extras) =>
      renderToolbarToggleButton(s, extras.backstage.shared.providers)
    ),
    menubutton: renderFromBridge(createMenuButton, (s, extras) =>
      renderMenuButton(s, 'tox-tbtn', extras.backstage, Optional.none())
    ),
    splitbutton: renderFromBridge(createSplitButton, (s, extras) =>
    ...
    grouptoolbarbutton: renderFromBridge(
      ...
      (s, extras, editor) => {
        ...
        const attributes = {
          [Attribute]: extras.backstage.shared.header.isPositionedAtTop()
    ...
    styleSelectButton: (editor, extras) =>
      createStylesButton(editor, extras.backstage),
    fontsizeSelectButton: (editor, extras) =>
      createFontSizeButton(editor, extras.backstage),
    fontSelectButton: (editor, extras) =>
      createFontFamilyButton(editor, extras.backstage),
    formatButton: (editor, extras) =>
      createBlocksButton(editor, extras.backstage),
    alignMenuButton: (editor, extras) =>
      createAlignButton(editor, extras.backstage)
  }
  const extractFrom = (spec, extras, editor) =>
  ...
  const lookupButton = (
    ...
    get$g(buttons, toolbarItem.toLowerCase())
      ...
      .fold(
        () =>
          get$g(bespokeButtons, toolbarItem.toLowerCase())
  ...
  const render$1 = (editor, uiComponents, rawUiConfig, backstage, args) => {
    ...
    editor.addQueryValueHandler('ToggleSidebar', () =>
      OuterContainer.whichSidebar(outerContainer)
    )
  ...
  const InlineHeader = (
    ...
    const updateChromePosition = () => {
      ...
      const top = isPositionedAtTop()
        ? Math.max(
            targetBounds.y - get$d(floatContainer.get().element) + offset,
            0
          )
        : targetBounds.bottom
  ...
  const setupEvents = (editor, targetElm, ui, toolbarPersist) => {
    ...
    elementLoad.set(
      capture(SugarElement.fromDom(editor.getBody()), 'load', resizeContent)
    )
    editor.on('remove', () => {
      elementLoad.clear()
    })
  ...
  const languageSpec = (editor) => {
    ...
    return settingsOpt.map((settings) => ({
      ...
      watcher: (editor, value, callback) =>
        editor.formatter.formatChanged('lang', callback, false, {
          value: value.code,
  ...
  const initAndShow$1 = (
    ...
    build(items, ItemResponse$1.CLOSE_ON_EXECUTE, backstage, false).map(
      (menuData) => {
        e.preventDefault()
        InlineView.showMenuAt(
          contextmenu,
  ...
  const show = (
    ...
    build(items, ItemResponse$1.CLOSE_ON_EXECUTE, backstage, true).map(
      (menuData) => {
        e.preventDefault()
  ...
  const getDimensions = (
    ...
    const dimensions = {}
    dimensions.height = calcCappedSize(
